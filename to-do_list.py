...
def AddTask():
    ...
    if len(task_str) == 0 :
        messagebox.showinfo('Task should contain minimum one charater !!')
    else:
        tasks.append(task_str)
        the_cursor.execute('insert into tasks values(?)',(task_str,))
        Update_list()
...
if __name__ == "__main__":
    ...
    header=tk.Frame(guiWindow,bg = "#98F5FF")
    functions_frame = tk.Frame(guiWindow,bg = "#98F5FF")
    listbox_frame = tk.Frame(guiWindow,bg = "#98F5FF")
    
    header.pack(fill="both")
    functions_frame.pack(side="left",expand=True,fill="both")
    listbox_frame.pack(side="right",expand = True,fill="both")
    
    header_label = ttk.Label(header, text = "YOUR TO-DO LIST", font = ("Helvetica", "25", "bold") , 
                             background = "#98F5FF", foreground = "#8B4513")
    ...
    task_field = ttk.Entry(functions_frame,font=("Consolas","12"),width =18, 
                           background = "#FFF8DC",foreground = "#A52A2A")
    ...
    delete_all_button = ttk.Button (functions_frame,text="Delete All Tasks ",width = 26, 
                                    command = delete_all)
    ...
    task_listbox = tk.Listbox(listbox_frame,width=26,height=20,selectmode ='SINGLE',
                              background ="#FFFFFF",foreground="#000000",selectbackground="#CD853F",selectforeground="#FFFFFF")
